...
if [ ! -f "backend/.deps_installed" ]; then
    ...
    "$PYTHON_BIN" -m pip install -r backend/requirements.txt && touch backend/.deps_installed
...
"$PYTHON_BIN" -m uvicorn backend.server:app --host 0.0.0.0 --port 8000 > backend.log 2>&1 &
BACKEND_PID=$!
...
NETWORK_URL=$(grep -m1 "Network:" frontend.log | sed 's/.*Network:[[:space:]]*//')
