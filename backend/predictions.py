...
def generate_daily_predictions(
    ...
    """Generate Top 5 Daily Trade predictions."""
    analyses = []
    for stock in stocks:
    ...
    # Sort by FCS descending, take top 5 actionable signals
    analyses.sort(key=lambda a: a.fcs.score, reverse=True)
    top = [a for a in analyses if a.fcs.score >= 50][:5]
...
def _classify_daily_signal(analysis) -> str:
    """Classify into D1-D5 signal types."""
...
def generate_weekly_predictions(
    ...
    """Generate Top 10 Weekly Position predictions."""
    analyses = []
    for stock in stocks:
        hist = histories.get(stock.symbol, [])
    ...
    analyses.sort(key=lambda a: a.fcs.score, reverse=True)
    top = [a for a in analyses if a.fcs.score >= 45][:10]
...
def generate_monthly_predictions(
    ...
    """Generate Top 5 Monthly Conviction Picks."""
    analyses = []
    for stock in stocks:
        hist = histories.get(stock.symbol, [])
