...
export function computeTML(stock: Stock, indicators: TechnicalIndicators): { score: number; details: string[] } {
  let score = 50;
  const details: string[] = [];
...
export function computeSSIL(stock: Stock, indicators: TechnicalIndicators): { score: number; sis: number; details: string[] } {
  ...
  const sentimentPressure = Math.min(
    12,
    Math.max(0, (indicators.stochRsi - 50) / 8) +
      Math.max(0, indicators.volumeRatio - 1) * 3 +
      Math.max(0, stock.changePercent) * 0.8
  );
  
  // SIS: 0-100
  const sis = Math.min(100, Math.round((volumeAnomaly + momentumSignal + priceVelocity + sentimentPressure) * 10) / 10);
...
export function computeGTBIL(stock: Stock, indicators: TechnicalIndicators): { score: number; bmr: number; details: string[] } {
  ...
  const liquidityFactor = Math.min(1, (stock.volume * Math.max(stock.cmp, 1)) / Math.max(stock.marketCap * 0.002, 1));
  const stabilityFactor = Math.max(0, 1 - Math.min(Math.abs(stock.changePercent), 10) / 10);
  const blueChipBonus = ['Commercial Bank', 'Hydropower', 'Insurance'].includes(stock.sector) ? 0.08 : 0;
  ...
  const bmr = Math.min(
    ...
    Math.max(
      5,
      Math.round(
        (marketCapFactor * 20 + liquidityFactor * 15 + stabilityFactor * 10 + blueChipBonus * 100 + indicators.volumeRatio * 8) * 10
      ) / 10
    )
  );
...
export function checkOverrides(stock: Stock, sis: number, bmr: number): OverrideCondition[] {
...
export function computeFCS(
  ...
  const tml = computeTML(stock, indicators);
  ...
  const overrides = checkOverrides(stock, ssil.sis, gtbil.bmr);
